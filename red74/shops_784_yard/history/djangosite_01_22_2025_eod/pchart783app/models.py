...
class CommonFields(models.Model):
    ...
    def __str__(self):
        return f"Common Fields (Updated: {self.updated_at}, Created: {self.created_at})"
    class Meta:
        abstract = True
...
class Survey(CommonFields):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='pchart783app_surveys')
    ...
    part_numbers = models.ManyToManyField('PartNumber', blank=True, related_name='pchart783app_surveys')
    
    def __str__(self):
        return f"Survey {self.id}: {self.name} (Cell: {self.cell}) by {self.user}"
...
class Answer(CommonFields):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='pchart783app_answers')
...
class QuestionAnswer(CommonFields):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='pchart783app_questionanswers')
    survey = models.ForeignKey(Survey, on_delete=models.CASCADE)
    question_name = models.TextField(help_text="Name of the question from the survey JSON", default= None)
    ...
    response = models.TextField(help_text="The answer to the question",blank=True, null=True)
