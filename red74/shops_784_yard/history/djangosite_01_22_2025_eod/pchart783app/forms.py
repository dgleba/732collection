...
class DynamicForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        for field in form_structure:
            ...
            if field_type == 'text':
                self.fields[field_name] = forms.CharField(label=field_label,required=required,help_text=help_text)
                ...
                self.fields[field_name] = forms.CharField(
                    label=field_label, required=required,
                    help_text=help_text,
                    widget=forms.Textarea(attrs={'class': 'form-control','rows': 3,'style': 'width: 200%;'})
                )
                ...
                choice_tbl_model = apps.get_model('pchart783app', 'choice_tbl')
                choices = choice_tbl_model.objects.filter(choice_tbl_grouping=grouping).exclude(choice_name='').values_list('choice_name', 'choice_name')
                self.fields[field_name] = forms.ChoiceField(
                    label=field_label, 
                    help_text=help_text, 
                    required=required, 
                ...
                part_number_model = apps.get_model('pchart783app', 'PartNumber')
                survey_model = apps.get_model('pchart783app', 'Survey')
                help_text=help_text,
                if survey_id:
                    try:
