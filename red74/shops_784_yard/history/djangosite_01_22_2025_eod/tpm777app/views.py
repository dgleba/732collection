...
def display_survey(request, survey_id):
            ...
            for field_name, user_answer in form.cleaned_data.items():
                ...
                if question:
                    ...
                    if question.get("type") == "choice":
                        issue = request.POST.get(f"issue_{field_name}", '').strip()
                        action_taken = request.POST.get(f"action_{field_name}", '').strip()
                        if issue:
                            response_data[field_name]["issue"] = issue
                        if action_taken:
                            response_data[field_name]["action_taken"] = action_taken

            # Save the response in the Answer model only if there are valid answers
            if response_data:
                ...
                return redirect('tpm777app:survey_success')
    else:
    ...
    return render(request, 'display_survey.html', {'form': form, 'survey_instance': survey_instance})
...
def survey_list(request):
    ...
    if request.headers.get('HX-Request'):
        ...
        if target == 'answers':
            return render(request, "partials/answers_list.html", {"answers": answers})
...
def show_conditional_fields(request, field_name):
    autofill_url = reverse('tpm777app:autofill_action', args=[field_name])
