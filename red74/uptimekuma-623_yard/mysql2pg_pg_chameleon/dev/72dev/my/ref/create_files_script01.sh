...
ref.
https://claude.ai/public/artifacts/f48e90be-cd63-4735-b24e-2a9f929049ff
...
services:
  ...
  chameleon:
    ...
    depends_on:
      ...
      - postgres
    volumes:
      - ./config:/root/.pg_chameleon/configuration
...
RUN mkdir -p /root/.pg_chameleon/configuration
...
# pg_chameleon configuration file
...
- The chameleon container uses `tail -f /dev/null` to keep running, allowing you to exec into it
...
- For production use, consider running the replica as a background service
EOF
